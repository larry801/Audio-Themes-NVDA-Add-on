...
class CreateNewDialog(BaseEditorDialog):
	...
	def getAudioTheme(self):
		...
		audioTheme = audioThemeHandler.makeAudioTheme(name=self.name, infoDict=self.infoDict)
		audioTheme.directory = tempDir
		return audioTheme

	def makeTitle(self):
		# Translators: The title of the main dialog being used to create a new audio theme.
		return _("Creating {themeName}, a new audio theme").format(themeName=self.audioTheme.name)
	...
	def onPackClick(self, evt):
		...
		gui.messageBox(
		  ...
		  _("The audio theme package was created successfully. You can find the package at: {filename}").format(filename=filename),
...
class CreaterDialog(gui.SettingsDialog):
	...
	def makeSettings(self, settingsSizer):
		...
		instructionLabel = wx.StaticText(self,-1,label=_("&Please enter the details you want to be shown in the themes manager for your new theme and then click the OK button to start adding sounds to your audio theme."))
		...
		mainSizer.AddMany([(instructionLabel),
		  (nameLabel), (self.nameEdit),
		  (authorLabel), (self.authorEdit),
		  (descLabel), (self.descEdit)])
